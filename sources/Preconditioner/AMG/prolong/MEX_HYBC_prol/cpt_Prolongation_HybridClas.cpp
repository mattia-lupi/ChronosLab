...
//       cpt_Prolongation_HybridClas(level, np, vecstart, nn_A, nt_A,
...
//
// ALL FIXES APPLIED (identical pattern to cpt_Prolongation_Classical)
// -----------------------------------------------------------------------
...
//         coef_S, fcnodes) — latent UB: mxGetPr returns double*, casting
//         to int* is undefined behaviour. Replaced with TypedArray<int32_t>
//         copies via std::vector<int32_t>.
//
...
//
// [NEW-4] Output arrays iat_I and ja_I stored as double — preserved from
...
//
// [NEW-6] Commented-out dead code (prol_parm params block) removed.
// -----------------------------------------------------------------------
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
static int cpt_Prolongation_HybridClas(const int level, const int nthreads,
...
{
    return HybridClas_prolongation(level, nthreads, vecstart,
...
public:
    ...
    void operator()(ArgumentList outputs, ArgumentList inputs) override
    {
        ...
        const int nn_I  = static_cast<int>(TypedArray<double>(inputs[10])[0]);
        const int nc_I  = static_cast<int>(TypedArray<double>(inputs[11])[0]);
        ...
        //   coef_A is the only genuine double array input.
        // -----------------------------------------------------------------------
        ...
        const TypedArray<int32_t> fcnodes_arr  = inputs[ 9];   // [NEW-1] int, not double
        ...
        int ierr = cpt_Prolongation_HybridClas(
                       level, np,
                       vecstart_vec.data(),
                       nn_A, nt_A,
                       iat_A_vec.data(),  ja_A_vec.data(),  coef_A_vec.data(),
                       coef_S_vec.data(), fcnodes_vec.data(),
                       nn_I, nc_I,
        ...
        if (!iat_I_raw || !ja_I_raw || !coef_I_raw)
            throwError("HybridClas_Prol:nullPointer",
                       "Kernel returned a null pointer — likely an allocation failure.");

        // [FIX-A] Route kernel error through MATLAB exception machinery
        if (ierr != 0)
            throwError("HybridClas_Prol:computeError",
                       "cpt_Prolongation_HybridClas returned error code: " +
                       std::to_string(ierr));
...
private:
    ...
    {
        if (inputs.size() != 12)
            throwError("HybridClas_Prol:badInputCount",
                       "Expected 12 input arguments, got " +
                       std::to_string(inputs.size()) + ".");

        if (outputs.size() != 4)
            throwError("HybridClas_Prol:badOutputCount",
                       "Expected 4 output arguments, got " +
                       std::to_string(outputs.size()) + ".");

        // Scalar double inputs: level(0), np(1), nn_A(3), nt_A(4), nn_I(10), nc_I(11)
        for (std::size_t i : {0u, 1u, 3u, 4u, 10u, 11u})
            if (inputs[i].getType() != ArrayType::DOUBLE ||
                inputs[i].getNumberOfElements() != 1)
                throwError("HybridClas_Prol:badScalar",
                           "Input argument " + std::to_string(i + 1) +
                           " must be a real double scalar.");

        // int32 array inputs: vecstart(2), iat_A(5), ja_A(6), coef_S(8), fcnodes(9)
        for (std::size_t i : {2u, 5u, 6u, 8u, 9u})
            if (inputs[i].getType() != ArrayType::INT32)
                throwError("HybridClas_Prol:badArray",
                           "Input argument " + std::to_string(i + 1) +
                           " must be an int32 array.");

        // double array input: coef_A(7)
        if (inputs[7].getType() != ArrayType::DOUBLE)
            throwError("HybridClas_Prol:badArray",
                       "Input argument 8 (coef_A) must be a double array.");
    }
