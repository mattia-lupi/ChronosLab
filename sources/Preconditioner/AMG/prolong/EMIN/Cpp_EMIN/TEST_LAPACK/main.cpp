...
double **AA   = (double**) malloc( mm*sizeof(double*) );
double *AAbuf = (double*) malloc( (mm*nn)*sizeof(double) );
for (int i = 0; i < mm; i++){
   AA[i] = &(AAbuf[k]);
   k += nn;
}
...
ierr = LAPACKE_dorgqr_work(LAPACK_COL_MAJOR,l_nn,l_mm,l_kk,AAbuf,l_nn,tau,&query_work_2,lwork);
...
// Create Q
ierr = LAPACKE_dorgqr_work(LAPACK_COL_MAJOR,l_nn,l_mm,l_kk,AAbuf,l_nn,tau,work,lwork);
