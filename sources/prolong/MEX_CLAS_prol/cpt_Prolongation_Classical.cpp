...
//       cpt_Prolongation_Classical(level, np, vecstart, nn_A, nt_A,
...
//
// ALL FIXES APPLIED (consistent with cpt_Prolongation_EXTI and BAMG)
// -----------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
static int cpt_Prolongation_Classical(const int level, const int nthreads,
...
{
    return Classical_prolongation(level, nthreads, vecstart,
...
public:
    ...
    void operator()(ArgumentList outputs, ArgumentList inputs) override
    {
        ...
        int ierr = cpt_Prolongation_Classical(
                       level, np,
                       vecstart_vec.data(),
                       nn_A, nt_A,
        ...
        if (!iat_I_raw || !ja_I_raw || !coef_I_raw)
            throwError("Classical_Prol:nullPointer",
                       "Kernel returned a null pointer — likely an allocation failure.");

        // [FIX-A] Route kernel error through MATLAB exception machinery
        if (ierr != 0)
            throwError("Classical_Prol:computeError",
                       "cpt_Prolongation_Classical returned error code: " +
                       std::to_string(ierr));
...
private:
    ...
    {
        if (inputs.size() != 12)
            throwError("Classical_Prol:badInputCount",
                       "Expected 12 input arguments, got " +
                       std::to_string(inputs.size()) + ".");

        if (outputs.size() != 4)
            throwError("Classical_Prol:badOutputCount",
        ...
        for (std::size_t i : {0u, 1u, 3u, 4u, 10u, 11u})
            if (inputs[i].getType() != ArrayType::DOUBLE ||
                ...
                throwError("Classical_Prol:badScalar",
        ...
        for (std::size_t i : {2u, 5u, 6u, 8u, 9u})
            if (inputs[i].getType() != ArrayType::INT32)
                throwError("Classical_Prol:badArray",
                           "Input argument " + std::to_string(i + 1) +
